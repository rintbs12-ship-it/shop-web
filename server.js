...
app.put('/api/admin/settings', requireAuth, (req, res) => {
  uploadSettings.fields([{ name: 'logo', maxCount: 1 }, { name: 'banner', maxCount: 1 }])(req, res, async (err) => {
    if (err) return res.status(400).json({ success: false, message: err.message });
    try {
      const { shop_name, shop_description, shop_phone, shop_address, facebook_link, telegram_link, banner_title, banner_desc, remove_banner } = req.body;
      ...
      const tabNumbers = Array.from({ length: 9 }, (_, index) => index + 4);
      const tabValues = tabNumbers.flatMap(n => [
        req.body[`tab${n}_name`] || `Tab ${n}`,
        req.body[`tab${n}_icon`] || 'fas fa-tag',
        req.body[`tab${n}_enabled`] ? 1 : 0,
      ]);
      const tabAssignments = tabNumbers.map((n, index) => {
        const parameter = 10 + (index * 3);
        return `tab${n}_name=$${parameter}, tab${n}_icon=$${parameter + 1}, tab${n}_enabled=$${parameter + 2}`;
      }).join(',\n          ');
      const bannerParameter = 10 + tabValues.length;
      await pool.query(`
